...
// src/react/ObjectIdProvider.tsx
...
async function resolveEnv(cfg) {
  ...
  const client$1 = new client.IotaClient({ url: client.getFullnodeUrl(net) });
  const keyPair = ed25519.Ed25519Keypair.deriveKeypairFromSeed(cfg.seed);
...
async function signAndExecute(client, keyPair, tx, opts) {
  try {
    if (opts.useGasStation) {
      ...
      try {
        const txEffect2 = await attemptWithGasStation(
          opts.network,
          client,
          keyPair,
          tx,
        ...
        const status2 = txEffect2.effects?.status;
        const ok2 = status2?.status === "success";
        return {
          success: !!ok2,
          txDigest: txEffect2.digest,
          status: status2,
          error: ok2 ? void 0 : status2?.error,
          txEffect: txEffect2
        };
      } catch (e1) {
...
async function add_approver_did(api, params) {
  ...
  tx.moveCall({
    arguments: [
      ...
      tx.object(document),
      tx.pure.string(new_approver_did)
    ],
    target: moveFunction
  });
  tx.setGasBudget(1e7);
...
async function update_publisher_did(api, params) {
  ...
  tx.moveCall({
    arguments: [
      tx.object(controllerCap),
      tx.object(document),
      tx.pure.string(new_publisher_did)
    ],
    target: moveFunction
  });
  tx.setGasBudget(1e7);
...
var C = react.createContext(void 0);
function ObjectIdProvider({ config, children }) {
  const api = react.useMemo(() => {
    if (!config) return null;
    return createObjectIdApi(config);
  }, [config]);
  return /* @__PURE__ */ jsxRuntime.jsx(C.Provider, { value: { api }, children });
}
