...
export function asJsonString(v) {
...
export async function resolveEnv(cfg) {
    ...
    const objIdx = Number.isFinite(cfg.objectDefaultPackageVersion) ? Number(cfg.objectDefaultPackageVersion) : 0;
    const docIdx = Number.isFinite(cfg.documentDefaultPackageVersion) ? Number(cfg.documentDefaultPackageVersion) : 0;
    ...
    if (!policy)
        throw new Error("Policy object not found via GraphQL.");
    return {
        client,
        keyPair,
        sender,
